...
// our users json
  ...
  login(user:string,password:string){
    ...
    const expires = new Date(new Date().getTime() + 30*oneMinute).toUTCString(); // times to expires cookie
    this.currentUser = this.registeredUsers.find(item => item.userName == user && item.password == password) // where the current user is saved
